...
```{r}
datacsv3 = as.data.frame(temptest$PassengerId)  
datacsv3$Survived = logpridect$data$response
```
```{r}
write.csv(datacsv3, file = "Submision13.csv", row.names = FALSE)
```
```{r}
set.seed(1)
...
```
```{r}
write.csv(datacsv2, file = "Submision12.csv", row.names = FALSE)
```
...
```
```{r}
write.csv(datacsv, file = "Submision10.csv", row.names = FALSE)
```
...
```
```{r}
write.csv(datacsv0, file = "Submision04.csv", row.names = FALSE)
```
